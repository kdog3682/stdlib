...
VimLParser.prototype.check_missing_endfunction = function (
    ends,
    pos,
) {
...
VimLParser.prototype.check_missing_endif = function (ends, pos) {
    if (
        this.context[0].type == NODE_IF ||
        this.context[0].type == NODE_ELSEIF ||
        this.context[0].type == NODE_ELSE
    ) {
        throw Err(
            viml_printf("E171: Missing :endif:    %s", ends),
            pos,
        )
    }
}

VimLParser.prototype.check_missing_endtry = function (
    ends,
    pos,
) {
    if (
        this.context[0].type == NODE_TRY ||
        this.context[0].type == NODE_CATCH ||
        this.context[0].type == NODE_FINALLY
    ) {
        throw Err(
            viml_printf("E600: Missing :endtry:    %s", ends),
            pos,
        )
    }
}

VimLParser.prototype.check_missing_endwhile = function (
    ends,
    pos,
) {
    if (this.context[0].type == NODE_WHILE) {
        throw Err(
            viml_printf("E170: Missing :endwhile:    %s", ends),
            pos,
        )
    }
}

VimLParser.prototype.check_missing_endfor = function (
    ends,
    pos,
) {
    if (this.context[0].type == NODE_FOR) {
        throw Err(
            viml_printf("E170: Missing :endfor:    %s", ends),
            pos,
        )
    }
}
...
VimLParser.prototype.parse = function (reader) {
    ...
    this.check_missing_endfunction(
        "TOPLEVEL",
        this.reader.getpos(),
    )
...
VimLParser.prototype.parse_command_modifiers = function () {
    ...
    while (TRUE) {
        ...
        this.reader.skip_white()
        if (
            viml_stridx("aboveleft", k) == 0 && viml_len(k) >= 3
        ) {
            // abo\%[veleft]
            viml_add(modifiers, { "name": "aboveleft" })
        } else if (
            viml_stridx("belowright", k) == 0 && viml_len(k) >= 3
        ) {
            // bel\%[owright]
            viml_add(modifiers, { "name": "belowright" })
        } else if (
            viml_stridx("browse", k) == 0 && viml_len(k) >= 3
        ) {
            // bro\%[wse]
            viml_add(modifiers, { "name": "browse" })
        } else if (
            viml_stridx("botright", k) == 0 && viml_len(k) >= 2
        ) {
            // bo\%[tright]
            viml_add(modifiers, { "name": "botright" })
        } else if (
            viml_stridx("confirm", k) == 0 && viml_len(k) >= 4
        ) {
            // conf\%[irm]
            viml_add(modifiers, { "name": "confirm" })
        } else if (
            viml_stridx("keepmarks", k) == 0 && viml_len(k) >= 3
        ) {
            // kee\%[pmarks]
            viml_add(modifiers, { "name": "keepmarks" })
        } else if (
            viml_stridx("keepalt", k) == 0 && viml_len(k) >= 5
        ) {
            // keepa\%[lt]
            viml_add(modifiers, { "name": "keepalt" })
        } else if (
            viml_stridx("keepjumps", k) == 0 && viml_len(k) >= 5
        ) {
            // keepj\%[umps]
            viml_add(modifiers, { "name": "keepjumps" })
        } else if (
            viml_stridx("keeppatterns", k) == 0 &&
            viml_len(k) >= 5
        ) {
            // keepp\%[atterns]
            viml_add(modifiers, { "name": "keeppatterns" })
        } else if (
            viml_stridx("hide", k) == 0 && viml_len(k) >= 3
        ) {
            // hid\%[e]
            ...
            viml_stridx("lockmarks", k) == 0 && viml_len(k) >= 3
        ) {
            // loc\%[kmarks]
            viml_add(modifiers, { "name": "lockmarks" })
        } else if (
            viml_stridx("leftabove", k) == 0 && viml_len(k) >= 5
        ) {
            // lefta\%[bove]
            viml_add(modifiers, { "name": "leftabove" })
        } else if (
            viml_stridx("noautocmd", k) == 0 && viml_len(k) >= 3
        ) {
            // noa\%[utocmd]
            viml_add(modifiers, { "name": "noautocmd" })
        } else if (
            viml_stridx("noswapfile", k) == 0 && viml_len(k) >= 3
        ) {
            // :nos\%[wapfile]
            viml_add(modifiers, { "name": "noswapfile" })
        } else if (
            viml_stridx("rightbelow", k) == 0 && viml_len(k) >= 6
        ) {
            // rightb\%[elow]
            viml_add(modifiers, { "name": "rightbelow" })
        } else if (
            viml_stridx("sandbox", k) == 0 && viml_len(k) >= 3
        ) {
            // san\%[dbox]
            viml_add(modifiers, { "name": "sandbox" })
        } else if (
            viml_stridx("silent", k) == 0 && viml_len(k) >= 3
        ) {
            // sil\%[ent]
            if (c == "!") {
                this.reader.get()
            ...
            if (d != "") {
                viml_add(modifiers, {
                    "name": "tab",
                    "count": viml_str2nr(d, 10),
                })
            } else {
                viml_add(modifiers, { "name": "tab" })
            }
        } else if (
            viml_stridx("topleft", k) == 0 && viml_len(k) >= 2
        ) {
            // to\%[pleft]
            viml_add(modifiers, { "name": "topleft" })
        } else if (
            viml_stridx("unsilent", k) == 0 && viml_len(k) >= 3
        ) {
            // uns\%[ilent]
            viml_add(modifiers, { "name": "unsilent" })
        } else if (
            viml_stridx("vertical", k) == 0 && viml_len(k) >= 4
        ) {
            // vert\%[ical]
            viml_add(modifiers, { "name": "vertical" })
        } else if (
            viml_stridx("verbose", k) == 0 && viml_len(k) >= 4
        ) {
            // verb\%[ose]
            if (d != "") {
                viml_add(modifiers, {
...
VimLParser.prototype.parse_command = function () {
    ...
    if (
        this.reader.peekn(1) == "" || this.reader.peekn(1) == '"'
    ) {
        if (
            !viml_empty(this.ea.modifiers) ||
            !viml_empty(this.ea.range)
        ) {
    ...
    if (
        this.reader.peekn(1) == "!" &&
        this.ea.cmd.name != "substitute" &&
        this.ea.cmd.name != "smagic" &&
        this.ea.cmd.name != "snomagic"
    ) {
        this.reader.getn(1)
    ...
    if (
        !viml_eqregh(this.ea.cmd.flags, "\\<BANG\\>") &&
        this.ea.forceit &&
        !viml_eqregh(this.ea.cmd.flags, "\\<USERCMD\\>")
    ) {
    ...
    if (
        this.ea.cmd.name == "write" ||
        this.ea.cmd.name == "update"
    ) {
        if (this.reader.p(0) == ">") {
            if (this.reader.p(1) != ">") {
    ...
    if (
        viml_eqregh(this.ea.cmd.flags, "\\<EDITCMD\\>") &&
        !this.ea.usefilter
    ) {
        this.parse_argcmd()
    }
...
VimLParser.prototype.find_command = function () {
    ...
    for (var __i4 = 0; __i4 < __c4.length; ++__i4) {
        var x = __c4[__i4]
        if (
            viml_stridx(x.name, name) == 0 &&
            viml_len(name) >= x.minlen
        ) {
            var cmd = x
            break
        }
    ...
    if (
        (cmd === NIL || cmd.name == "Print") &&
        viml_eqregh(name, "^[A-Z]")
    ) {
        name += this.reader.read_alnum()
        var cmd = {
            "name": name,
            "flags": "USERCMD",
            "parser": "parse_cmd_usercmd",
        }
    }
...
VimLParser.prototype.parse_comment = function () {
    ...
    if (c != '"') {
        throw Err(
            viml_printf("unexpected character: %s", c),
            npos,
        )
    }
...
VimLParser.prototype.parse_trail = function () {
    ...
    if (c == "<EOF>") {
        ...
        throw Err(
            viml_printf("E488: Trailing characters: %s", c),
            this.reader.getpos(),
        )
    }
}
...
VimLParser.prototype.parse_cmd_common = function () {
    ...
    if (
        viml_eqregh(this.ea.cmd.flags, "\\<TRLBAR\\>") &&
        !this.ea.usefilter
    ) {
        var end = this.separate_nextcmd()
    } else if (
        this.ea.cmd.name == "!" ||
        this.ea.cmd.name == "global" ||
        this.ea.cmd.name == "vglobal" || this.ea.usefilter
    ) {
        while (TRUE) {
            var end = this.reader.getpos()
...
VimLParser.prototype.separate_nextcmd = function () {
    if (
        this.ea.cmd.name == "vimgrep" ||
        this.ea.cmd.name == "vimgrepadd" ||
        this.ea.cmd.name == "lvimgrep" ||
        this.ea.cmd.name == "lvimgrepadd"
    ) {
    ...
    while (TRUE) {
        ...
        if (c == "<EOF>" || c == "<EOL>") {
            ...
            c == '"' &&
                ...
                (this.ea.cmd.name != "@" &&
                        this.ea.cmd.name != "*" ||
                    this.reader.getpos() != this.ea.argpos) &&
                (this.ea.cmd.name != "redir" ||
                    this.reader.getpos().i !=
                        this.ea.argpos.i + 1 ||
                    pc != "@")
        ) {
...
VimLParser.prototype.skip_vimgrep_pat = function () {
    if (this.reader.peekn(1) == "") {
        ...
        while (
            this.reader.p(0) == "g" || this.reader.p(0) == "j"
        ) {
            this.reader.getn(1)
        }
    }
...
VimLParser.prototype.parse_cmd_function = function () {
    ...
    if (tokenizer.peek().type == TOKEN_PCLOSE) {
        ...
        while (TRUE) {
            ...
            if (token.type == TOKEN_IDENTIFIER) {
                ...
                if (
                    iswhite(this.reader.p(0)) &&
                    tokenizer.peek().type == TOKEN_COMMA
                ) {
                    throw Err(
    ...
    while (TRUE) {
        ...
        if (key == "") {
            ...
            throw Err(
                viml_printf("unexpected token: %s", key),
                epos,
            )
        }
    }
...
VimLParser.prototype.parse_heredoc = function (prefix) {
    ...
    throw Err(
        viml_printf("E990: Missing end marker '%s'", node.op),
        this.reader.getpos(),
    )
}
...
VimLParser.prototype.parse_cmd_continue = function () {
    if (
        this.find_context(NODE_WHILE) == -1 &&
        this.find_context(NODE_FOR) == -1
    ) {
        throw Err(
...
VimLParser.prototype.parse_lvalue_func = function () {
    ...
    if (
        ...
        node.type == NODE_SUBSCRIPT || node.type == NODE_DOT ||
        node.type == NODE_OPTION || node.type == NODE_ENV ||
        node.type == NODE_REG
    ) {
        return node
    }
...
VimLParser.prototype.parse_lvalue = function () {
    ...
    if (
        node.type == NODE_IDENTIFIER ||
        node.type == NODE_CURLYNAME ||
        node.type == NODE_SUBSCRIPT || node.type == NODE_SLICE ||
        node.type == NODE_DOT || node.type == NODE_OPTION ||
        node.type == NODE_ENV || node.type == NODE_REG
    ) {
        return node
    }
    throw Err("Invalid Expression", node.pos)
...
VimLParser.prototype.neovim_additional_commands = [{
    "name": "rshada",
    "minlen": 3,
...
VimLParser.prototype.neovim_removed_commands = [{
    "name": "Print",
    "minlen": 1,
...
VimLParser.prototype.builtin_commands = [{
    "name": "append",
    "minlen": 1,
    "flags": "BANG|RANGE|ZEROR|TRLBAR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_append",
}, {
    "name": "abbreviate",
    "minlen": 2,
    "flags": "EXTRA|TRLBAR|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "abclear",
    "minlen": 3,
    "flags": "EXTRA|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "aboveleft",
    "minlen": 3,
    ...
    "name": "all",
    "minlen": 2,
    "flags": "BANG|RANGE|COUNT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "amenu",
    "minlen": 2,
    "flags":
        "RANGE|ZEROR|EXTRA|TRLBAR|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "anoremenu",
    "minlen": 2,
    ...
    "name": "args",
    "minlen": 2,
    "flags": "BANG|FILES|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "argadd",
    "minlen": 4,
    "flags": "BANG|RANGE|ZEROR|FILES|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "argdelete",
    "minlen": 4,
    "flags": "BANG|RANGE|FILES|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "argdo",
    "minlen": 5,
    "flags": "BANG|NEEDARG|EXTRA|NOTRLCOM|RANGE|DFLALL|EXPAND",
    "parser": "parse_cmd_common",
}, {
    "name": "argdedupe",
    "minlen": 6,
    ...
    "minlen": 4,
    "flags":
        "BANG|NEEDARG|RANGE|ZEROR|FILES|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "argglobal",
    "minlen": 4,
    ...
    "name": "argument",
    "minlen": 4,
    "flags": "BANG|RANGE|COUNT|EXTRA|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "ascii",
    "minlen": 2,
    "flags": "TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "autocmd",
    "minlen": 2,
    "flags": "BANG|EXTRA|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "augroup",
    "minlen": 3,
    "flags": "BANG|WORD1|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "aunmenu",
    "minlen": 3,
    ...
    "minlen": 1,
    "flags":
        "BANG|RANGE|BUFNAME|BUFUNL|COUNT|EXTRA|CMDARG|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "bNext",
    "minlen": 2,
    "flags": "BANG|RANGE|COUNT|CMDARG|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "ball",
    "minlen": 2,
    "flags": "RANGE|COUNT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "badd",
    "minlen": 3,
    "flags": "NEEDARG|FILE1|CMDARG|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "balt",
    "minlen": 4,
    ...
    "name": "bdelete",
    "minlen": 2,
    "flags": "BANG|RANGE|BUFNAME|COUNT|EXTRA|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "behave",
    "minlen": 2,
    "flags": "BANG|NEEDARG|WORD1|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "belowright",
    "minlen": 3,
    ...
    "name": "bfirst",
    "minlen": 2,
    "flags": "BANG|RANGE|CMDARG|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "blast",
    "minlen": 2,
    ...
    "name": "browse",
    "minlen": 3,
    "flags": "NEEDARG|EXTRA|NOTRLCOM|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "buffers",
    "minlen": 7,
    "flags": "BANG|EXTRA|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "bufdo",
    "minlen": 4,
    ...
    "name": "bwipeout",
    "minlen": 2,
    "flags": "BANG|RANGE|BUFNAME|BUFUNL|COUNT|EXTRA|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "change",
    "minlen": 1,
    "flags":
        "BANG|WHOLEFOLD|RANGE|COUNT|TRLBAR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "cNext",
    "minlen": 2,
    "flags": "RANGE|COUNT|TRLBAR|BANG",
    "parser": "parse_cmd_common",
}, {
    "name": "cNfile",
    "minlen": 3,
    ...
    "name": "caddbuffer",
    "minlen": 3,
    "flags": "RANGE|WORD1|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "caddexpr",
    "minlen": 5,
    "flags": "NEEDARG|WORD1|NOTRLCOM|EXPR_ARG",
    "parser": "parse_cmd_common",
}, {
    "name": "caddfile",
    "minlen": 5,
    ...
    "minlen": 3,
    "flags":
        "RANGE|NEEDARG|EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_call",
}, {
    "name": "catch",
    "minlen": 3,
    "flags": "EXTRA|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_catch",
}, {
    "name": "cbuffer",
    "minlen": 2,
    "flags": "BANG|RANGE|WORD1|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "cbefore",
    "minlen": 3,
    ...
    "name": "cd",
    "minlen": 2,
    "flags": "BANG|FILE1|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "cdo",
    "minlen": 3,
    ...
    "minlen": 2,
    "flags":
        "TRLBAR|RANGE|WHOLEFOLD|EXTRA|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "cexpr",
    "minlen": 3,
    "flags": "NEEDARG|WORD1|NOTRLCOM|EXPR_ARG|BANG",
    "parser": "parse_cmd_common",
}, {
    "name": "cfile",
    "minlen": 2,
    ...
    "name": "changes",
    "minlen": 7,
    "flags": "TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "checkpath",
    "minlen": 3,
    "flags": "TRLBAR|BANG|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "checktime",
    "minlen": 6,
    "flags": "RANGE|BUFNAME|COUNT|EXTRA|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "chistory",
    "minlen": 3,
    ...
    "name": "close",
    "minlen": 3,
    "flags": "BANG|RANGE|COUNT|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "clearjumps",
    "minlen": 3,
    ...
    "minlen": 2,
    "flags":
        "RANGE|WHOLEFOLD|EXTRA|TRLBAR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "colder",
    "minlen": 3,
    ...
    "name": "colorscheme",
    "minlen": 4,
    "flags": "WORD1|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "command",
    "minlen": 3,
    "flags": "EXTRA|BANG|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "comclear",
    "minlen": 4,
    ...
    "name": "compiler",
    "minlen": 4,
    "flags": "BANG|TRLBAR|WORD1|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "continue",
    "minlen": 3,
    "flags": "TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_continue",
}, {
    "name": "confirm",
    "minlen": 4,
    "flags": "NEEDARG|EXTRA|NOTRLCOM|CMDWIN|LOCK_OK",
    ...
    "minlen": 4,
    "flags":
        "EXTRA|BANG|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_const",
}, {
    "name": "copen",
    "minlen": 4,
    "flags": "RANGE|COUNT|TRLBAR",
    ...
    "name": "cquit",
    "minlen": 2,
    "flags": "RANGE|COUNT|ZEROR|TRLBAR|BANG",
    "parser": "parse_cmd_common",
}, {
    "name": "crewind",
    "minlen": 2,
    ...
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|REGSTR|COUNT|TRLBAR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "delmarks",
    "minlen": 4,
    ...
    "name": "debug",
    "minlen": 3,
    "flags": "NEEDARG|EXTRA|NOTRLCOM|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "debuggreedy",
    "minlen": 6,
    "flags": "RANGE|ZEROR|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "def",
    "minlen": 3,
    "flags": "EXTRA|BANG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "defer",
    "minlen": 4,
    "flags":
        "NEEDARG|EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "defcompile",
    "minlen": 4,
    "flags": "SBOXOK|CMDWIN|LOCK_OK|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "delcommand",
    "minlen": 4,
    "flags": "NEEDARG|WORD1|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "delfunction",
    "minlen": 4,
    "flags": "BANG|NEEDARG|WORD1|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_delfunction",
}, {
    "name": "display",
    "minlen": 2,
    "flags": "EXTRA|NOTRLCOM|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "diffupdate",
    "minlen": 3,
    ...
    "name": "disassemble",
    "minlen": 4,
    "flags": "BANG|EXTRA|NEEDARG|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "djump",
    "minlen": 2,
    ...
    "name": "dlist",
    "minlen": 2,
    "flags": "BANG|RANGE|DFLALL|WHOLEFOLD|EXTRA|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "doautocmd",
    "minlen": 2,
    ...
    "name": "drop",
    "minlen": 2,
    "flags": "FILES|CMDARG|NEEDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "dsearch",
    "minlen": 2,
    ...
    "name": "edit",
    "minlen": 1,
    "flags": "BANG|FILE1|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "earlier",
    "minlen": 2,
    "flags": "TRLBAR|EXTRA|NOSPC|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "echo",
    "minlen": 2,
    "flags": "EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_echo",
}, {
    "name": "echoerr",
    "minlen": 5,
    "flags": "EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_echoerr",
}, {
    "name": "echohl",
    "minlen": 5,
    "flags": "EXTRA|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_echohl",
}, {
    "name": "echomsg",
    "minlen": 5,
    "flags": "EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_echomsg",
}, {
    "name": "echoconsole",
    "minlen": 5,
    "flags": "EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    ...
    "minlen": 2,
    "flags":
        "NEEDARG|EXTRA|TRLBAR|NOTRLCOM|RANGE|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "endif",
    "minlen": 2,
    ...
    "minlen": 4,
    "flags": "TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_endfunction",
}, {
    "name": "endfor",
    "minlen": 5,
    "flags": "TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_endfor",
}, {
    "name": "endtry",
    "minlen": 4,
    "flags": "TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_endtry",
}, {
    "name": "endwhile",
    "minlen": 4,
    "flags": "TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_endwhile",
}, {
    "name": "enew",
    "minlen": 3,
    "flags": "BANG|TRLBAR",
    ...
    "minlen": 3,
    "flags": "EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_execute",
}, {
    "name": "exit",
    "minlen": 3,
    "flags":
        "RANGE|WHOLEFOLD|BANG|FILE1|ARGOPT|DFLALL|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "export",
    "minlen": 3,
    "flags": "EXTRA|NOTRLCOM|EXPR_ARG|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "exusage",
    "minlen": 3,
    ...
    "name": "file",
    "minlen": 1,
    "flags": "RANGE|ZEROR|BANG|FILE1|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "files",
    "minlen": 5,
    ...
    "name": "find",
    "minlen": 3,
    "flags": "RANGE|BANG|FILE1|CMDARG|ARGOPT|TRLBAR|NEEDARG",
    "parser": "parse_cmd_common",
}, {
    "name": "final",
    "minlen": 4,
    "flags": "EXTRA|NOTRLCOM|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "finally",
    "minlen": 6,
    "flags": "TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_finally",
}, {
    "name": "finish",
    "minlen": 4,
    "flags": "TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_finish",
}, {
    "name": "first",
    "minlen": 3,
    "flags": "EXTRA|BANG|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "fixdel",
    "minlen": 3,
    ...
    "name": "fold",
    "minlen": 2,
    "flags": "RANGE|WHOLEFOLD|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "foldclose",
    "minlen": 5,
    "flags": "RANGE|BANG|WHOLEFOLD|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "folddoopen",
    "minlen": 5,
    ...
    "minlen": 3,
    "flags": "EXTRA|NOTRLCOM|EXPR_ARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_for",
}, {
    "name": "function",
    "minlen": 2,
    "flags": "EXTRA|BANG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_function",
}, {
    "name": "global",
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|BANG|EXTRA|DFLALL|SBOXOK|CMDWIN|LOCK_OK|NONWHITE_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "goto",
    "minlen": 2,
    "flags": "RANGE|COUNT|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "grep",
    "minlen": 2,
    "flags": "RANGE|BANG|NEEDARG|EXTRA|NOTRLCOM|TRLBAR|XFILE",
    "parser": "parse_cmd_common",
}, {
    "name": "grepadd",
    "minlen": 5,
    ...
    "name": "gui",
    "minlen": 2,
    "flags": "BANG|FILES|CMDARG|ARGOPT|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "gvim",
    "minlen": 2,
    ...
    "name": "helptags",
    "minlen": 5,
    "flags": "NEEDARG|FILES|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "hardcopy",
    "minlen": 2,
    "flags": "RANGE|COUNT|EXTRA|EXPAND|TRLBAR|DFLALL|BANG",
    "parser": "parse_cmd_common",
}, {
    "name": "highlight",
    "minlen": 2,
    "flags": "BANG|EXTRA|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "hide",
    "minlen": 3,
    "flags": "BANG|RANGE|COUNT|EXTRA|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "history",
    "minlen": 3,
    ...
    "name": "insert",
    "minlen": 1,
    "flags": "BANG|RANGE|TRLBAR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_insert",
}, {
    "name": "iabbrev",
    "minlen": 2,
    "flags": "EXTRA|TRLBAR|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    ...
    "name": "import",
    "minlen": 3,
    "flags": "EXTRA|NOTRLCOM|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "inoremap",
    "minlen": 3,
    ...
    "minlen": 1,
    "flags":
        "BANG|RANGE|WHOLEFOLD|COUNT|FLAGS|TRLBAR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "jumps",
    "minlen": 2,
    ...
    "minlen": 1,
    "flags":
        "RANGE|WORD1|TRLBAR|SBOXOK|CMDWIN|LOCK_OK|NONWHITE_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "keepmarks",
    "minlen": 2,
    ...
    "name": "list",
    "minlen": 1,
    "flags": "RANGE|WHOLEFOLD|COUNT|FLAGS|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "lNext",
    "minlen": 2,
    ...
    "name": "loadkeymap",
    "minlen": 5,
    "flags": "CMDWIN|LOCK_OK",
    "parser": "parse_cmd_loadkeymap",
}, {
    "name": "lockmarks",
    "minlen": 3,
    "flags": "NEEDARG|EXTRA|NOTRLCOM",
    ...
    "name": "lockvar",
    "minlen": 5,
    "flags": "BANG|EXTRA|NEEDARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_lockvar",
}, {
    "name": "lolder",
    "minlen": 3,
    "flags": "RANGE|COUNT|TRLBAR",
    ...
    "name": "lua",
    "minlen": 3,
    "flags": "RANGE|EXTRA|NEEDARG|CMDWIN|LOCK_OK|RESTRICT",
    "parser": "parse_cmd_lua",
}, {
    "name": "luado",
    "minlen": 4,
    "flags":
        "RANGE|DFLALL|EXTRA|NEEDARG|CMDWIN|LOCK_OK|RESTRICT",
    "parser": "parse_cmd_common",
}, {
    "name": "luafile",
    "minlen": 4,
    "flags": "RANGE|FILE1|NEEDARG|CMDWIN|LOCK_OK|RESTRICT",
    "parser": "parse_cmd_common",
}, {
    "name": "lvimgrep",
    "minlen": 2,
    "flags":
        "RANGE|BANG|NEEDARG|EXTRA|NOTRLCOM|TRLBAR|XFILE|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "lvimgrepadd",
    "minlen": 9,
    ...
    "name": "mark",
    "minlen": 2,
    "flags": "RANGE|WORD1|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "make",
    "minlen": 3,
    ...
    "name": "map",
    "minlen": 3,
    "flags": "BANG|EXTRA|TRLBAR|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "mapclear",
    "minlen": 4,
    "flags": "EXTRA|BANG|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "marks",
    "minlen": 5,
    ...
    "name": "match",
    "minlen": 3,
    "flags": "RANGE|EXTRA|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "menu",
    "minlen": 2,
    "flags":
        "RANGE|ZEROR|BANG|EXTRA|TRLBAR|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "menutranslate",
    "minlen": 5,
    ...
    "name": "messages",
    "minlen": 3,
    "flags": "EXTRA|TRLBAR|RANGE|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "mkexrc",
    "minlen": 2,
    ...
    "minlen": 2,
    "flags":
        "RANGE|EXTRA|DFLALL|NEEDARG|CMDWIN|LOCK_OK|SBOXOK|RESTRICT",
    "parser": "parse_cmd_mzscheme",
}, {
    "name": "mzfile",
    "minlen": 3,
    "flags": "RANGE|FILE1|NEEDARG|CMDWIN|LOCK_OK|RESTRICT",
    ...
    "name": "next",
    "minlen": 1,
    "flags": "RANGE|BANG|FILES|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "nbkey",
    "minlen": 2,
    ...
    "name": "new",
    "minlen": 3,
    "flags": "BANG|FILE1|RANGE|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "nmap",
    "minlen": 2,
    ...
    "minlen": 4,
    "flags":
        "RANGE|BANG|EXTRA|NEEDARG|NOTRLCOM|CTRLV|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "number",
    "minlen": 2,
    ...
    "name": "oldfiles",
    "minlen": 2,
    "flags": "BANG|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "omap",
    "minlen": 2,
    ...
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|COUNT|FLAGS|TRLBAR|CMDWIN|LOCK_OK|SBOXOK",
    "parser": "parse_cmd_common",
}, {
    "name": "packadd",
    "minlen": 2,
    "flags": "BANG|FILE1|NEEDARG|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "packloadall",
    "minlen": 5,
    ...
    "name": "perl",
    "minlen": 2,
    "flags": "RANGE|EXTRA|DFLALL|NEEDARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_perl",
}, {
    "name": "perldo",
    "minlen": 5,
    "flags": "RANGE|EXTRA|DFLALL|NEEDARG|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "pedit",
    "minlen": 3,
    ...
    "name": "pop",
    "minlen": 2,
    "flags": "RANGE|BANG|COUNT|TRLBAR|ZEROR",
    "parser": "parse_cmd_common",
}, {
    "name": "popup",
    "minlen": 4,
    "flags": "NEEDARG|EXTRA|BANG|TRLBAR|NOTRLCOM|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "ppop",
    "minlen": 2,
    ...
    "name": "previous",
    "minlen": 4,
    "flags": "EXTRA|RANGE|COUNT|BANG|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "promptfind",
    "minlen": 3,
    ...
    "name": "ptag",
    "minlen": 2,
    "flags": "RANGE|BANG|WORD1|TRLBAR|ZEROR",
    "parser": "parse_cmd_common",
}, {
    "name": "ptNext",
    "minlen": 3,
    "flags": "RANGE|BANG|TRLBAR|ZEROR",
    "parser": "parse_cmd_common",
}, {
    "name": "ptfirst",
    "minlen": 3,
    ...
    "minlen": 2,
    "flags":
        "RANGE|WHOLEFOLD|BANG|REGSTR|TRLBAR|ZEROR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "pwd",
    "minlen": 2,
    ...
    "name": "qall",
    "minlen": 2,
    "flags": "BANG|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "read",
    "minlen": 1,
    "flags":
        "BANG|RANGE|WHOLEFOLD|FILE1|ARGOPT|TRLBAR|ZEROR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "recover",
    "minlen": 3,
    ...
    "name": "redir",
    "minlen": 4,
    "flags": "BANG|FILES|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "redraw",
    "minlen": 4,
    ...
    "name": "registers",
    "minlen": 3,
    "flags": "EXTRA|NOTRLCOM|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "resize",
    "minlen": 3,
    "flags": "RANGE|TRLBAR|WORD1|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "retab",
    "minlen": 3,
    "flags":
        "TRLBAR|RANGE|WHOLEFOLD|DFLALL|BANG|WORD1|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "return",
    "minlen": 4,
    "flags": "EXTRA|NOTRLCOM|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_return",
}, {
    "name": "rewind",
    "minlen": 3,
    "flags": "EXTRA|BANG|CMDARG|ARGOPT|TRLBAR",
    ...
    "name": "runtime",
    "minlen": 2,
    "flags": "BANG|NEEDARG|FILES|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "ruby",
    "minlen": 3,
    "flags": "RANGE|EXTRA|NEEDARG|CMDWIN|LOCK_OK|RESTRICT",
    "parser": "parse_cmd_ruby",
}, {
    "name": "rubydo",
    "minlen": 5,
    "flags":
    ...
    "name": "substitute",
    "minlen": 1,
    "flags": "RANGE|WHOLEFOLD|EXTRA|CMDWIN|LOCK_OK|NONWHITE_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "sNext",
    "minlen": 2,
    ...
    "name": "saveas",
    "minlen": 3,
    "flags": "BANG|FILE1|ARGOPT|CMDWIN|LOCK_OK|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "sbuffer",
    "minlen": 2,
    ...
    "name": "sbNext",
    "minlen": 3,
    "flags": "RANGE|COUNT|CMDARG|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "sball",
    "minlen": 3,
    ...
    "name": "set",
    "minlen": 2,
    "flags": "BANG|TRLBAR|EXTRA|CMDWIN|LOCK_OK|SBOXOK",
    "parser": "parse_cmd_common",
}, {
    "name": "setfiletype",
    "minlen": 4,
    "flags": "TRLBAR|EXTRA|NEEDARG|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "setglobal",
    "minlen": 4,
    ...
    "name": "sfind",
    "minlen": 2,
    "flags": "BANG|FILE1|RANGE|CMDARG|ARGOPT|TRLBAR|NEEDARG",
    "parser": "parse_cmd_common",
}, {
    "name": "sfirst",
    "minlen": 4,
    ...
    "name": "sign",
    "minlen": 3,
    "flags": "NEEDARG|RANGE|EXTRA|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "silent",
    "minlen": 3,
    "flags": "NEEDARG|EXTRA|BANG|NOTRLCOM|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "sleep",
    "minlen": 2,
    "flags": "BANG|RANGE|COUNT|EXTRA|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "slast",
    "minlen": 3,
    ...
    "name": "source",
    "minlen": 2,
    "flags": "BANG|FILE1|TRLBAR|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "sort",
    "minlen": 3,
    ...
    "name": "spellgood",
    "minlen": 3,
    "flags": "BANG|RANGE|NEEDARG|EXTRA|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "spelldump",
    "minlen": 6,
    ...
    "name": "smile",
    "minlen": 3,
    "flags": "TRLBAR|CMDWIN|LOCK_OK|SBOXOK",
    "parser": "parse_cmd_common",
}, {
    "name": "t",
    "minlen": 1,
    ...
    "minlen": 4,
    "flags":
        "BANG|RANGE|ZEROR|EXTRA|NOSPC|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "tabdo",
    "minlen": 4,
    "flags": "NEEDARG|EXTRA|NOTRLCOM|RANGE|DFLALL|EXPAND",
    "parser": "parse_cmd_common",
}, {
    "name": "tabedit",
    "minlen": 4,
    "flags": "BANG|FILE1|RANGE|ZEROR|CMDARG|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "tabfind",
    "minlen": 4,
    "flags":
        "BANG|FILE1|RANGE|ZEROR|CMDARG|ARGOPT|NEEDARG|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "tabfirst",
    "minlen": 6,
    ...
    "name": "tabmove",
    "minlen": 4,
    "flags": "RANGE|ZEROR|EXTRA|NOSPC|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "tablast",
    "minlen": 4,
    ...
    "name": "tearoff",
    "minlen": 2,
    "flags": "NEEDARG|EXTRA|TRLBAR|NOTRLCOM|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "terminal",
    "minlen": 3,
    "flags": "RANGE|BANG|FILES|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "tfirst",
    "minlen": 2,
    ...
    "name": "throw",
    "minlen": 2,
    "flags": "EXTRA|NEEDARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_throw",
}, {
    "name": "tjump",
    "minlen": 2,
    "flags": "BANG|TRLBAR|WORD1",
    ...
    "name": "undo",
    "minlen": 1,
    "flags": "RANGE|COUNT|ZEROR|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "undojoin",
    "minlen": 5,
    ...
    "minlen": 3,
    "flags": "BANG|EXTRA|NEEDARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_unlet",
}, {
    "name": "unlockvar",
    "minlen": 4,
    "flags": "BANG|EXTRA|NEEDARG|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_unlockvar",
}, {
    "name": "unmap",
    "minlen": 3,
    "flags": "BANG|EXTRA|TRLBAR|NOTRLCOM|CTRLV|CMDWIN|LOCK_OK",
    ...
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|EXTRA|DFLALL|CMDWIN|LOCK_OK|NONWHITE_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "var",
    "minlen": 2,
    ...
    "minlen": 4,
    "flags":
        "NEEDARG|RANGE|EXTRA|NOTRLCOM|SBOXOK|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "vertical",
    "minlen": 4,
    ...
    "name": "wNext",
    "minlen": 2,
    "flags": "RANGE|WHOLEFOLD|BANG|FILE1|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "wall",
    "minlen": 2,
    ...
    "name": "wincmd",
    "minlen": 4,
    "flags": "NEEDARG|WORD1|RANGE|CMDWIN|LOCK_OK",
    "parser": "parse_wincmd",
}, {
    "name": "windo",
    "minlen": 4,
    ...
    "name": "wnext",
    "minlen": 2,
    "flags": "RANGE|BANG|FILE1|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "wprevious",
    "minlen": 2,
    ...
    "name": "wqall",
    "minlen": 3,
    "flags": "BANG|FILE1|ARGOPT|TRLBAR",
    "parser": "parse_cmd_common",
}, {
    "name": "wundo",
    "minlen": 2,
    ...
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|REGSTR|COUNT|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "z",
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|BANG|EXTRA|FLAGS|TRLBAR|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "!",
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|BANG|FILES|CMDWIN|LOCK_OK|NONWHITE_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "#",
    "minlen": 1,
    ...
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|EXTRA|CMDWIN|LOCK_OK|MODIFY|NONWHITE_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "*",
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|EXTRA|TRLBAR|CMDWIN|LOCK_OK|NONWHITE_OK",
    "parser": "parse_cmd_common",
}, {
    "name": "<",
    "minlen": 1,
    "flags":
        "RANGE|WHOLEFOLD|COUNT|FLAGS|TRLBAR|CMDWIN|LOCK_OK|MODIFY",
    "parser": "parse_cmd_common",
}, {
    "name": "=",
    "minlen": 1,
    "flags": "RANGE|TRLBAR|DFLALL|FLAGS|CMDWIN|LOCK_OK",
    "parser": "parse_cmd_common",
}, {
    "name": ">",
    "minlen": 1,
...
VimLParser.prototype.builtin_functions = [{
    "name": "abs",
    "min_argc": 1,
    "max_argc": 1,
    "argtype": "FEARG_1",
...
ExprParser.prototype.parse_expr1 = function () {
    ...
    if (token.type == TOKEN_QUESTION) {
        ...
        if (token.type != TOKEN_COLON) {
            throw Err(
                viml_printf("unexpected token: %s", token.value),
                token.pos,
            )
        }
    ...
    while (TRUE) {
        ...
        if (!iswhite(c) && token.type == TOKEN_SQOPEN) {
            ...
            if (token.type != TOKEN_POPEN) {
                throw Err(
                    "E107: Missing parentheses: lambda",
                    token.pos,
                )
            }
...
ExprParser.prototype.parse_expr9 = function () {
    ...
    if (token.type == TOKEN_NUMBER) {
        ...
        token.type == TOKEN_COPEN || token.type == TOKEN_LITCOPEN
    ) {
        ...
        if (
            ...
            var lambda = token2.type == TOKEN_ARROW ||
                token2.type == TOKEN_COMMA
        }
            ...
            while (TRUE) {
                if (token.type == TOKEN_ARROW) {
                    ...
                    if (!isargname(token.value)) {
                        ...
                        viml_has_key(named, token.value)
                    ) {
                        throw Err(
                            viml_printf(
                                "E853: Duplicate argument name: %s",
                    ...
                    if (
                        iswhite(this.reader.p(0)) &&
                        this.tokenizer.peek().type == TOKEN_COMMA
                    ) {
                        throw Err(
                            "E475: Invalid argument: White space is not allowed before comma",
                            this.reader.getpos(),
        ...
        while (1) {
            var key = is_litdict
                ? this.tokenizer.parse_dict_literal_key()
                : this.parse_expr1()
            var token = this.tokenizer.get()
            if (token.type == TOKEN_CCLOSE) {
        ...
        FALSE &&
        (token.type == TOKEN_COLON || token.type == TOKEN_SHARP)
    ) {
        ...
        token.type == TOKEN_LT &&
        viml_equalci(this.reader.peekn(4), "SID>")
    ) {
        this.reader.seek_set(pos)
        var node = this.parse_identifier()
    } else if (
        token.type == TOKEN_IS || token.type == TOKEN_ISCS ||
        token.type == TOKEN_ISNOT || token.type == TOKEN_ISNOTCS
    ) {
        this.reader.seek_set(pos)
        var node = this.parse_identifier()
...
ExprParser.prototype.parse_dot = function (token, left) {
    if (
        ...
        left.type != NODE_CURLYNAME && left.type != NODE_DICT &&
        left.type != NODE_SUBSCRIPT && left.type != NODE_CALL &&
        left.type != NODE_DOT
    ) {
        return NIL
    }
...
ExprParser.prototype.parse_identifier = function () {
    ...
    if (
        viml_len(curly_parts) == 1 &&
        curly_parts[0].type == NODE_CURLYNAMEPART
    ) {
...
ExprParser.prototype.parse_curly_parts = function () {
    ...
    var pos = this.reader.getpos()
    if (
        c == "<" && viml_equalci(this.reader.peekn(5), "<SID>")
    ) {
    ...
    while (TRUE) {
        ...
        if (isnamec(c)) {
            ...
            if (c != "}") {
                throw Err(
                    viml_printf("unexpected token: %s", c),
                    this.reader.getpos(),
                )
            }
            this.reader.seek_cur(1)
...
function LvalueParser() {
    ExprParser.apply(this, arguments)
    this.__init__.apply(this, arguments)
}
...
StringReader.prototype.__init__ = function (lines) {
    ...
    while (lnum < viml_len(lines)) {
        ...
        while (
            lnum + 1 < viml_len(lines) &&
            viml_eqregh(lines[lnum + 1], "^\\s*\\\\")
        ) {
            var skip = TRUE
            var col = 0
...
Compiler.prototype.out = function () {
    ...
    if (viml_len(a000) == 1) {
        ...
        viml_add(
            this.lines,
            this.indent[0] + viml_printf.apply(null, a000),
        )
    }
}
...
Compiler.prototype.compile_function = function (node) {
    ...
    var rlist = node.rlist.map((function (vval) {
        return this.compile(vval)
    }).bind(this))
    var default_args = node.default_args.map((function (vval) {
        return this.compile(vval)
    }).bind(this))
    if (!viml_empty(rlist)) {
        var remaining = FALSE
...
Compiler.prototype.compile_unlet = function (node) {
    var list = node.list.map((function (vval) {
        return this.compile(vval)
    }).bind(this))
...
Compiler.prototype.compile_slice = function (node) {
    var r0 = node.rlist[0] === NIL
        ? "nil"
        : this.compile(node.rlist[0])
    var r1 = node.rlist[1] === NIL
        ? "nil"
        : this.compile(node.rlist[1])
    return viml_printf(
        "(slice %s %s %s)",
        this.compile(node.left),
        r0,
...
Compiler.prototype.compile_list = function (node) {
    var value = node.value.map((function (vval) {
        return this.compile(vval)
    }).bind(this))
    if (viml_empty(value)) {
        return "(list)"
...
Compiler.prototype.compile_heredoc = function (node) {
    if (viml_empty(node.rlist)) {
        ...
        var rlist = "(list " +
            viml_join(
                node.rlist.map((function (vval) {
                    return this.escape_string(vval.value)
                }).bind(this)),
                " ",
            ) + ")"
    }
...
RegexpParser.prototype.get_token_sq = function (pre) {
    ...
    while (TRUE) {
        ...
        if (
            startc != 0 && this.reader.p(0) == "-" &&
            !this.isend(this.reader.p(1)) &&
            !(this.reader.p(1) == "\\" &&
                this.reader.p(2) == "n")
        ) {
            this.reader.seek_cur(1)
...
RegexpParser.prototype.get_token_sq_coll_element = function () {
    if (
        this.reader.p(0) == "[" && this.reader.p(1) == "." &&
        !this.isend(this.reader.p(2)) &&
        this.reader.p(3) == "." && this.reader.p(4) == "]"
    ) {
...
class Visitor {
    ...
    visit(ast, ...args) {
        if (ast.type in this) {
            return this[ast.type](ast, ...args)
        }
    }
    [1](ast) {
        /* toplevel */
        return filter(ast.body.map(this.visit))
    }
    [4](ast, i, a) {
        /* function */
        const cur = ast.pos.i
        const pos  = a[i + 1] ? a[i+1].pos.i : this.s.length
        const text = this.s.slice(cur, pos)
...
function vimFunctionLibrary(s) {
    const v = new Visitor(s)
    const ast = main(s)
    console.log(ast)
    const res = v.visit(ast)
    return res
}
console.log(vimFunctionLibrary(s))
